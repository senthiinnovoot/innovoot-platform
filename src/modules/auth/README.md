...
Sign in, sign up, password reset, session management, and route guarding.
...
```text
modules/auth/
├── components/   # Business-specific UI (e.g. AuthCard, AuthTable) — not shared/ui
...
- Other modules (e.g. `orders` needing customer data) must import from
  `modules/auth` (its `index.ts`), never from
  `modules/auth/services/...` or any other internal path directly.

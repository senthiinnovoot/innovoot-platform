...
The catalog of services Innovoot's customers offer to their own customers.
...
```text
modules/services/
├── components/   # Business-specific UI (e.g. ServicesCard, ServicesTable) — not shared/ui
...
- Other modules (e.g. `orders` needing customer data) must import from
  `modules/services` (its `index.ts`), never from
  `modules/services/services/...` or any other internal path directly.

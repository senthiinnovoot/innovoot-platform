...
Public-facing page metadata, structured data, and search-visibility tooling.
...
```text
modules/seo/
├── components/   # Business-specific UI (e.g. SeoCard, SeoTable) — not shared/ui
...
- Other modules (e.g. `orders` needing customer data) must import from
  `modules/seo` (its `index.ts`), never from
  `modules/seo/services/...` or any other internal path directly.

...
Payment capture, invoicing, refunds, and reconciliation.
...
```text
modules/payments/
├── components/   # Business-specific UI (e.g. PaymentsCard, PaymentsTable) — not shared/ui
...
- Other modules (e.g. `orders` needing customer data) must import from
  `modules/payments` (its `index.ts`), never from
  `modules/payments/services/...` or any other internal path directly.

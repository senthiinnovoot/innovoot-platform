...
```text
modules/dashboard/
├── components/   # Business-specific UI (e.g. DashboardCard, DashboardTable) — not shared/ui
...
- Other modules (e.g. `orders` needing customer data) must import from
  `modules/dashboard` (its `index.ts`), never from
  `modules/dashboard/services/...` or any other internal path directly.

...
Campaigns, email/SMS outreach, promotions.
...
```text
modules/marketing/
├── components/   # Business-specific UI (e.g. MarketingCard, MarketingTable) — not shared/ui
...
- Other modules (e.g. `orders` needing customer data) must import from
  `modules/marketing` (its `index.ts`), never from
  `modules/marketing/services/...` or any other internal path directly.
